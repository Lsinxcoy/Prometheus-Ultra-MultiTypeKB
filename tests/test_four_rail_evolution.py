...
from prometheus_ultra.mechanisms.source_fetcher import fetch_repo_overview, fetch_arxiv_fulltext
...
def test_t3_mechanism_extractor_register(omega, monkeypatch):
    ...
    monkeypatch.setattr(
        "prometheus_ultra.mechanisms.mechanism_extractor.fetch_repo_overview",
...
def test_t4_mechanism_compiler_register(omega, monkeypatch):
    ...
    monkeypatch.setattr(
        "prometheus_ultra.mechanisms.mechanism_compiler.fetch_arxiv_fulltext",
...
def test_p2_rumination_routing_and_fuel(omega):
    ...
    from prometheus_ultra.foundation.schema import NodeType
    from prometheus_ultra.learning.knowledge_rumination import RuminationResult
    nid = omega.remember(
...
def test_p3_t3_t4_from_store_node(omega, monkeypatch):
    ...
    from prometheus_ultra.foundation.schema import NodeType, Node
...
def test_p5_maintain_type_floor(omega):
    ...
    from prometheus_ultra.foundation.schema import NodeType, Node
    from prometheus_ultra.foundation.schema import generate_uuidv7
...
def test_p6_activation_closure(omega, monkeypatch):
    ...
    monkeypatch.setattr(
        "prometheus_ultra.mechanisms.source_fetcher.fetch_repo_overview",
        lambda repo: "# r\nclass M:\n    pass",
    )
    monkeypatch.setattr(
        "prometheus_ultra.mechanisms.mechanism_extractor.fetch_repo_overview",
    ...
    monkeypatch.setattr(
        "prometheus_ultra.mechanisms.source_fetcher.fetch_arxiv_fulltext",
        lambda aid, max_chars=20000: "We propose X.",
    )
    monkeypatch.setattr(
        "prometheus_ultra.mechanisms.mechanism_compiler.fetch_arxiv_fulltext",
