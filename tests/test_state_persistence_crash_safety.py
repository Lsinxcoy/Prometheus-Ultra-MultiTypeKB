...
def test_load_missing_file_returns_empty_and_is_silent(tmp_state, caplog):
    omega = FakeOmega()
    with caplog.at_level(logging.ERROR, logger="prometheus_ultra.harness.state_persistence"):
        loaded = tmp_state.load(omega)
    assert loaded == {}
...
def test_load_corrupt_primary_recovers_from_backup(tmp_state, caplog):
    ...
    omega = FakeOmega()
    with caplog.at_level(logging.WARNING, logger="prometheus_ultra.harness.state_persistence"):
        loaded = tmp_state.load(omega)
