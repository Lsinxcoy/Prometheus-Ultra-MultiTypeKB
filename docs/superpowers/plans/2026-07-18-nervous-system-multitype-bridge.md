...
- Modify: `src/prometheus_ultra/learning/knowledge_rumination.py` — A1(rumination_completed publish + Telemetry schema 数据源)
- Modify: `src/prometheus_ultra/lifecycle/telemetry_pipeline.py` — A1(rumination schema)、C1(evolve 新诊断字段)
- Modify: `src/prometheus_ultra/lifecycle/cns_orchestrator.py` — A1(rumination→maintain 链)、C1(evolve_to_dream 质量门)
- Modify: `src/prometheus_ultra/lifecycle/cerebral_cortex.py` — A1(rumination 缺口监听，可选)
- Modify: `src/prometheus_ultra/lifecycle/autonomic_regulator.py` — A1(rumination 订阅)、B1(fitness 维度已在 _compute_fitness)、D1(capability_consumed 订阅)
- Modify: `src/prometheus_ultra/life.py` — B1(`_compute_fitness` 增维度)、D1(emit/apply 处 publish)
- Modify: `src/prometheus_ultra/integration/host_agent.py` — D1(adapter 内 publish capability_consumed)
...
**Files:**
- Modify: `src/prometheus_ultra/learning/knowledge_rumination.py` (ruminate 末尾 publish)
- Modify: `src/prometheus_ultra/lifecycle/telemetry_pipeline.py` (_PIPE_SCHEMAS 增 rumination + _resolve 解析)
...
import pytest
from prometheus_ultra.learning.knowledge_rumination import KnowledgeRuminationEngine, RuminationResult
...
- [ ] **Step 6: Commit**
```bash
cd /e/Prometheus-Ultra-MultiTypeKB && git add src/prometheus_ultra/learning/knowledge_rumination.py src/prometheus_ultra/lifecycle/telemetry_pipeline.py tests/test_nervous_system_multitype.py && git commit -m "feat(A1): rumination publishes rumination_completed + Telemetry schema"
```
...
**Files:**
- Modify: `src/prometheus_ultra/lifecycle/signal_fusion.py` (subscribe 列表加 rumination_completed)
- Modify: `src/prometheus_ultra/lifecycle/cns_orchestrator.py` (subscribe + rumination→maintain 触发链)
- Modify: `src/prometheus_ultra/lifecycle/autonomic_regulator.py` (subscribe + rumination 统计)
- Modify: `src/prometheus_ultra/lifecycle/cerebral_cortex.py` (subscribe 可选，本任务加以支持完整性)
- Test: `tests/test_nervous_system_multitype.py`
...
- [ ] **Step 8: Commit**
```bash
cd /e/Prometheus-Ultra-MultiTypeKB && git add src/prometheus_ultra/lifecycle/signal_fusion.py src/prometheus_ultra/lifecycle/cns_orchestrator.py src/prometheus_ultra/lifecycle/autonomic_regulator.py src/prometheus_ultra/lifecycle/cerebral_cortex.py tests/test_nervous_system_multitype.py && git commit -m "feat(A1): CNS/CC/AR/SFL subscribe rumination_completed"
```
...
**Files:**
- Modify: `src/prometheus_ultra/life.py` (`_compute_fitness` 4324 增三个维度)
- Test: `tests/test_nervous_system_multitype.py`
...
def test_fitness_includes_multitype_and_consumption():
    from prometheus_ultra.core.omega import Omega  # 或实际导入路径
...
- [ ] **Step 6: Commit**
```bash
cd /e/Prometheus-Ultra-MultiTypeKB && git add src/prometheus_ultra/life.py tests/test_nervous_system_multitype.py && git commit -m "feat(B1): _compute_fitness adds multitype/consumption/rumination dimensions"
```
...
**Files:**
- Modify: `src/prometheus_ultra/lifecycle/telemetry_pipeline.py` (`_PIPE_SCHEMAS["evolve"]` 增 camp/speculative/consensus 字段 + `_resolve` 解析)
- Modify: `src/prometheus_ultra/lifecycle/cns_orchestrator.py` (`_on_evolve` 或 `evolve_to_dream` 触发条件叠加质量门)
- Test: `tests/test_nervous_system_multitype.py`
...
- [ ] **Step 6: Commit**
```bash
cd /e/Prometheus-Ultra-MultiTypeKB && git add src/prometheus_ultra/lifecycle/telemetry_pipeline.py src/prometheus_ultra/lifecycle/cns_orchestrator.py tests/test_nervous_system_multitype.py && git commit -m "feat(C1): Telemetry evolve consensus/speculative + CNS quality gate"
```
...
**Files:**
- Modify: `src/prometheus_ultra/integration/host_agent.py` (`emit_capability`/`apply_capability` 成功时 publish `capability_consumed`)
- Modify: `src/prometheus_ultra/life.py` (T4 激活 emit 成功后亦可由 adapter 内 publish，无需重复)
- Modify: `src/prometheus_ultra/lifecycle/autonomic_regulator.py` (AR 订阅 `capability_consumed` 纳入 fitness 趋势)
- Test: `tests/test_nervous_system_multitype.py`
...
- [ ] **Step 6: Commit**
```bash
cd /e/Prometheus-Ultra-MultiTypeKB && git add src/prometheus_ultra/integration/host_agent.py src/prometheus_ultra/lifecycle/autonomic_regulator.py tests/test_nervous_system_multitype.py && git commit -m "feat(D1): capability consumption flows back to nervous system"
```
